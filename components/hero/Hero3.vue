...
const { notice, title, content, buttons, background } = props.data
</script>

<template>
  <section class="bg-center bg-no-repeat bg-gray-700 bg-blend-multiply  bg-[image:var(--image-url)]" :style="`--image-url: url('${background?.light}');`">
    <div class="px-4 mx-auto max-w-screen-xl text-center py-24 lg:py-56">
      ...
      <!-- Form -->
      <form class="w-full max-w-md mx-auto mb-10">
      ...
      <div class="flex flex-col space-y-4 sm:flex-row sm:justify-center sm:space-y-0 sm:space-x-4">
        <NuxtLink
          ...
          :key="button.label"
          :to="button.link"
          :class="button.class ? button.class : 'btn-outline text-white hover:text-gray-900'"
          class="btn mr-3"
        >
          {{ button.label }}
        </NuxtLink>
